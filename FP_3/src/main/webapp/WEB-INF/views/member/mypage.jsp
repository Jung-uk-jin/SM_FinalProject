			    ...
			    <div class="sidebar">
			        ...
			        <div class="wallet">
						...
						<a href="/myCoupon"><div class="wallet-item coupon">
						    <div class="wallet-label"><strong>적립금</strong></div>
						    <div class="wallet-amount">${mdto.member_mileage}원&nbsp;<i class="fa-solid fa-chevron-right" style="font-size: 16px; padding: 0;"></i></div>
						</div></a>
						<!-- 쿠폰 박스 -->
					        ...
					        <c:forEach var="orderItem" items="${orderItems}">
					            ...
					            <div class="order-header">
					                <div class="order-date">
					                    <fmt:formatDate value="${orderItem.order.order_date}" pattern="yyyy.MM.dd" />
					                </div>
					                <a><div class="order-detail-title">주문상세</div></a>
					            </div>
					            <!-- 주문 상품 정보 -->
					            <div class="order-item">
					                <img src="/images/${orderItem.shop.shop_image1}" alt="상품 이미지" class="product-image">
					                <div class="product-info">
					                    <p>${orderItem.shop.shop_title}</p>
					                    <span>${orderItem.shop.shop_content} / ${orderItem.orderItem_quantity}개</span>
					                    <strong><fmt:formatNumber value="${orderItem.order.totalPrice}" pattern="#,###"/>원</strong>
					                    <div class="actions">
										    <!-- 왼쪽: 주문 상태 메시지 -->
										    <div class="status-container">
										        <c:if test="${orderItem.order.order_status eq 'PENDING'}">
										             <p class="status-btn pending">결제 대기</p>
										        </c:if>
										        <c:if test="${orderItem.order.order_status eq 'PAID'}">
										             <p class="status-btn paid">결제 완료</p>
										        </c:if>
										        <c:if test="${orderItem.order.order_status eq 'PROCESSING'}">
										             <p class="status-btn processing">상품 준비중</p>
										        </c:if>
										        <c:if test="${orderItem.order.order_status eq 'SHIPPING'}">
										             <p class="status-btn shipping">배송 중</p>
										        </c:if>
										        <c:if test="${orderItem.order.order_status eq 'DELIVERED'}">
										             <p class="status-btn delivered">배송 완료</p>
										        </c:if>
										        <c:if test="${orderItem.order.order_status eq 'CANCELLED'}">
										             <p class="status-btn cancelled">주문 취소됨</p>
										        </c:if>
										        <c:if test="${orderItem.order.order_status eq 'RETURNED'}">
										             <p class="status-btn returned">반품 완료</p>
										        </c:if>
										        <c:if test="${orderItem.order.order_status eq 'REFUNDED'}">
										             <p class="status-btn refunded">환불 완료</p>
										        </c:if>
										        <c:if test="${orderItem.order.order_status eq 'FAILED'}">
										             <p class="status-btn failed">결제 실패</p>
										        </c:if>
										        <c:if test="${orderItem.order.order_status ne 'PENDING' 
										                      and orderItem.order.order_status ne 'PAID'
										                      and orderItem.order.order_status ne 'PROCESSING'
										                      and orderItem.order.order_status ne 'SHIPPING'
										                      and orderItem.order.order_status ne 'DELIVERED'
										                      and orderItem.order.order_status ne 'CANCELLED'
										                      and orderItem.order.order_status ne 'RETURNED'
										                      and orderItem.order.order_status ne 'REFUNDED'
										                      and orderItem.order.order_status ne 'FAILED'}">
										             <p class="status-btn unknown">기타 상태</p>
										        </c:if>
										    </div>
										    ...
										    <div class="buttons-container">
										        <!-- 배송 조회 버튼: 배송 중인 경우에만 -->
										        <c:if test="${orderItem.order.order_status eq 'SHIPPING'}">
										             <button id="look">배송 조회</button>
										        </c:if>
										        
										        <!-- 재구매 버튼: 배송 완료된 경우에만 -->
										        <c:if test="${orderItem.order.order_status eq 'DELIVERED'}">
										             <button id="onemore">재구매</button>
										        </c:if>
										        
										        <!-- 주문취소 버튼: 결제 완료 또는 상품 준비중 상태일 때 노출 -->
										        <c:if test="${orderItem.order.order_status eq 'PAID' or orderItem.order.order_status eq 'PROCESSING'}">
										             <button id="xxx">주문취소</button>
										        </c:if>
										    </div>
										</div>
