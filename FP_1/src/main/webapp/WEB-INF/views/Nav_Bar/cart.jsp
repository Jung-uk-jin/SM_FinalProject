...
<body>
    ...
    <!-- 장바구니 -->
		        ...
		        <c:choose>
		            <c:when test="${hasMerch}">
		                ...
		                <c:forEach var="cart" items="${cartList}">
		                    <c:forEach var="item" items="${cart.parsedCartItems}">
		                        <c:if test="${item['shop_category'] ne '티켓' and item['shop_category'] ne null}">
		                            <div class="cart-item" id="cart-${cart.cart_no}">
		                                <img src="/images/index_login/goods1.png" alt="상품 이미지" class="cart-item-img">
		                                <div class="cart-item-info">
		                                    <p class="cart-item-name">${item['shop_title']}</p>
		                                    <p class="cart-item-quantity"><fmt:formatNumber value="${item['shop_quantity']}" pattern="#,###"/>개</p>
		                                    <p class="cart-item-price"><strong><fmt:formatNumber value="${item['shop_price']}" pattern="#,###"/>원</strong></p>
		...
		$(document).on("click", ".cart-item-btn.delete", function () {
		    ...
		    let cartNo = $cartItem.attr("id").replace("cart-", "");

		    console.log("cartNo 값 확인:", cartNo);

		    if (!cartNo || isNaN(cartNo)) {
		            ...
		            success: function (data) {
		                if (data.success) {
		                    ...
		                    $cartItem.remove();
		                 	...
		                 	// 🔥 구매 금액 업데이트 (삭제된 상품 가격 반영)
		                    updatePaymentSummary(-itemPrice);

		                    // 🔥 남은 장바구니 아이템 개수 업데이트
		                    let countAll = $(".cart-container .cart-item").length;
		                    console.log("현재 남은 장바구니 상품 개수:", countAll);

		                    // 🔥 장바구니가 비었는지 확인
		                    if (countAll === 0) {
		                        console.log("🛑 장바구니가 비어 있음. 메시지 표시");
		                        $(".cart-container").html(`
		                                <h2>Merch</h2>
		                                <p>장바구니에 담긴 상품이 없습니다.</p>
		                                <button class="goShop" type="button">팬지샵(굿즈)으로 이동</button>
