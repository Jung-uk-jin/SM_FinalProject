...
<head>
 ...
 // 10. 무한스크롤
 $(document).ready(function () {
    let rowsPerLoad = 2; // 한 번에 보여줄 행 개수 (10개씩)
    let currentIndex = 2; // 현재까지 표시된 행 개수 (처음 2줄(10개)은 이미 보임)

    function showNextRows() {
        $(".artist-row:hidden").slice(0, rowsPerLoad).fadeIn(); // 숨겨진 다음 2줄 표시
        currentIndex += rowsPerLoad;
    }

    // 스크롤 이벤트 감지하여 추가 데이터 표시
    $(window).on("scroll", function () {
        if ($(window).scrollTop() + $(window).height() >= $(document).height() - 200) {
            showNextRows(); // 다음 데이터 표시
        }
    });
}); // 10번 종료
				        ...
				        <tbody id="artist-tbody">
				            <c:forEach items="${list}" var="adto" varStatus="status">
				                <c:if test="${status.index % 5 == 0}">
				                    <tr class="artist-row" <c:if test="${status.index >= 10}">style="display: none;"</c:if>>
				                </c:if>
				                ...
				                <td>
				                    <a href="/artist?artist_no=${adto.artist_no}">
				                        <div class="artist-card">
				                            ...
				                            <div class="logo"><img src="images/${adto.artist_logo_image}" alt="logo"></div>
			            ...
			            <ul>
			                <c:if test="${not empty recommendations}">
			                    <c:forEach var="artist" items="${recommendations}">
			                        <li class="recommend-artist-item">
			                            <span class="recommend-artist-name">${artist.name}</span>
			                            <img class="recommend-artist-img" src="${artist.imagePath}" alt="${artist.name}">
			                        </li>
			                    </c:forEach>
			                </c:if>
			            </ul>
