...
public interface NicknameRepository extends JpaRepository<NicknameDto, String>{
	...
	List<NicknameDto> findByNickname(@Param("nickname") String nickname);
 	...
 	@Query("SELECT n FROM NicknameDto n WHERE n.memberDto.member_nickname = :memberNickname AND n.artistDto.artist_no = :artistNo")
    NicknameDto findByMemberNicknameAndArtistNo(@Param("memberNickname") String memberNickname, @Param("artistNo") int artistNo);
