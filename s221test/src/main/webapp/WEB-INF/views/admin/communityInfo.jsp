...
<html dir="ltr" lang="en">
  <head>
    ...
    <![endif]-->
		<script>
			const deleteBtn = (fCommunityNo) =>{
				if(confirm("게시글을 삭제하시겠습니까?")){
					$.ajax({
						url:"/commudelete",
						type:"post",	
						data:{"community_no":fCommunityNo},
                      ...
                      <tbody>
	                      <c:if test="${not empty list}">
		              	    <c:forEach items="${list}" var="cdto">
						       <tr>
						         <td>${cdto.f_community_no}</td>
						         <td>${cdto.f_community_content}</td>
						         <td>${cdto.f_community_date}</td>
						         <td>0</td>
						         <td><button onclick="deleteBtn(${cdto.f_community_no})" class="register-button">삭제</button></td>
						       </tr>
						     </c:forEach>
					      </c:if>
